...
class _Clock:
    def __init__(self) -> None:
        self._value = datetime(2026, 5, 16, 10, 0, tzinfo=APP_TIMEZONE)
...
def test_retrieval_index_store_replaces_and_lists_source_chunks(tmp_path: Path) -> None:
    ...
    assert all(chunk.owner_user_id == "user_alpha" for chunk in listed)
    assert all(chunk.workspace_id == "workspace_alpha" for chunk in listed)
...
def _chunks(
    *,
    ...
    scope: RetrievalIndexScope = RetrievalIndexScope.USER_PRIVATE,
    sensitivity: RetrievalChunkSensitivity = RetrievalChunkSensitivity.PRIVATE,
) -> list[RetrievalChunk]:
    now = datetime(2026, 5, 16, 9, 0, tzinfo=APP_TIMEZONE)
