...
MID_TERM_SUMMARIZER_SYSTEM_PROMPT = (
    ...
    "- If an earlier proposal is later rejected or superseded, do not quote the obsolete proposal as an active fact. "
    "Summarize it as an abandoned/superseded candidate and foreground the latest decision.\n"
...
def build_mid_term_user_prompt(pack: MidTermEventPackLike) -> str:
    ...
    return (
        ...
        "10) Superseded proposal rule: if old text says a proposal and later text says final/改为/不接入/只作为, "
        "do not restate the old proposal verbatim; write it as an abandoned candidate and keep the final decision.\n"
        "11) Preserve original language and key terms verbatim, especially Chinese phrases like user names, goals, "
        "tool_call/tool_result rules, file names, and quoted tool results.\n"
        "12) Output strict JSON object only.\n"
