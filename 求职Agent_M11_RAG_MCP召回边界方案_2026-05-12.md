...
- 新增 `tests/test_retrieval_quality_eval.py`。
- 用同一套产品种子数据验证 5 类核心入口：二面准备、学习安排、复盘短板、简历与岗位匹配、当前会话文件。
- 每个评估用例都断言召回结果包含预期来源类型，citations 可追溯，`match_reason` 不为空。
- 当前没有发现必须立刻引入索引、embedding 或 MCP 才能解决的稳定失败类型。

结论：

- M11 第一阶段规则召回已足够支撑当前产品主链路。
- 暂缓 M11-4 索引 projection。
- 暂缓 embedding、MCP 和前端召回面板。
- 后续只有在真实使用中稳定出现“找不到、排序差、上下文太长、来源不完整、引用不清楚”等问题时，再重启 M11-4。
...
M11-1、M11-2、M11-3 和 M11 收口评估已完成。下一步不建议直接冲索引、embedding 或 MCP，应该回到产品主线体验：

```text
1. 把 retrieval_context_pack 用在面试准备、学习安排和求职项目推进中。
2. 观察真实链路里的召回失败类型。
