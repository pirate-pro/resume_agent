...
M21 候选版验收结论：主链路具备候选版演示条件，工作台关键交互已补充自动化覆盖；当前不新增主功能，只继续处理 P0 / P1 问题。
...
- 学习任务新建和打卡弹层关闭时，`TextEditingController` 可能在底部弹层退出动画结束前被释放，导致输入框生命周期异常。
- 修复方式是先关闭弹层，再发送 Agent 意图，并把 controller 释放延后一帧，同时保留 `BuildContext.mounted` 防护。
...
### P1：学习任务弹层关闭时的 controller 生命周期问题

现象：

- 工作台学习任务的新建和打卡动作在测试中暴露出弹层关闭动画尚未完成时释放输入 controller 的风险。

当前处理：

- 已修复。
- 已补充 widget test 覆盖学习任务推荐、主动新建和打卡入口。
