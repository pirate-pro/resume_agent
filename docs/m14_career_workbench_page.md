...
点击笔记后打开 `NoteEditorSheet`，可直接修改标题、摘要、标签、正文和笔记类型。

笔记类型只做轻量提示，不强制用户把笔记归为某个项目：

```text
note      -> 记录：想法、总结、过程记录、杂项
learning  -> 学习：知识点、学习计划、短板复盘
resource  -> 资料：面经、文章、链接、资产摘录
```

类型用于后续检索、学习建议和上下文选择，不替代用户手动关联求职项目或资产。

笔记正文以 Markdown 编辑为主，预览和更复杂的知识库召回放到后续阶段。
