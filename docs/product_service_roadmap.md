...
```text
1. 完成 M16 第一批：Agent 契约和确定性 runtime 测试。
2. 再评估是否需要前端在项目详情里强化面试复盘时间线。
