...
```text
1. 完成 M15 第一批：动作状态、结果定位、失败提示和定向测试。
2. 继续观察真实多 Agent 链路中，动作状态是否足够解释等待过程。
3. 暂不扩大 smoke 压力，优先保证主线体验闭环。
```
