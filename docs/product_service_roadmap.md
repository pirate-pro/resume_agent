...
- 候选版运行手册和最短演示路径已补齐：`docs/m21_candidate_runbook.md`。

### M22：候选版人工演示验收

目标：按 M21 运行手册完整走一遍人工演示路径，确认当前版本可以作为候选版基线。

方案文档：`docs/m22_candidate_demo_acceptance.md`

M22 的关键判断：

- 不新增主功能。
- 不接入 RAG / MCP、提醒系统、外部知识库写入或 memory 自动写入。
- 只记录并修复阻断候选版试用的 P0 / P1。
- 如果人工演示路径跑通且没有阻断问题，当前提交可以标记为候选版基线。

当前状态：

- M22 第一轮验收报告已完成：`docs/m22_candidate_demo_acceptance_report.md`。
- 前端首页首次截图发现旧 web-server 空白页，重启后桌面和窄屏首页恢复。
