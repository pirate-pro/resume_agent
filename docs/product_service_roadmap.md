...
```text
1. 进入 M15 规划：围绕工作台行动执行、状态刷新和下一步建议做闭环增强。
2. 优先评估“工作台动作执行中状态”和“执行后结果定位”，避免用户不知道新产物出现在哪里。
3. 保持低批次验证，不急着扩大 smoke 压力。
```
