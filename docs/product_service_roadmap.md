...
```text
1. 开发 M14-3 学习计划页，把学习计划、任务、短板和复盘从轻量展示升级成独立管理视图。
2. 做 M14 收口体验验收，确认工作台动作执行后能回到聊天、产物能回到工作台。
3. 再评估是否进入 M15：围绕工作台行动执行、状态刷新和下一步建议做闭环增强。
```
